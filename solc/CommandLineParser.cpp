...
{
	m_options.ignoreMissingInputFiles = (m_args.count(g_strIgnoreMissingFiles) > 0);
...
General Information)").c_str(),
	...
	m_options.withErrorIds = m_args.count(g_strErrorIds);
	...
	if (m_args.count(g_strOutputDir))
		m_options.outputDir = m_args.at(g_strOutputDir).as<string>();

	m_options.overwriteFiles = (m_args.count(g_strOverwrite) > 0);
	m_options.prettyJson = (m_args.count(g_strPrettyJson) > 0);

	static_assert(
		sizeof(m_options.selectedOutputs) == 15 * sizeof(bool),
		"Remember to update code below if you add/remove fields."
	);
	m_options.selectedOutputs.astCompactJson = (m_args.count(g_strAstCompactJson) > 0);
	m_options.selectedOutputs.asm_ = (m_args.count(g_strAsm) > 0);
	m_options.selectedOutputs.asmJson = (m_args.count(g_strAsmJson) > 0);
	m_options.selectedOutputs.opcodes = (m_args.count(g_strOpcodes) > 0);
	m_options.selectedOutputs.binary = (m_args.count(g_strBinary) > 0);
	m_options.selectedOutputs.binaryRuntime = (m_args.count(g_strBinaryRuntime) > 0);
	m_options.selectedOutputs.abi = (m_args.count(g_strAbi) > 0);
	m_options.selectedOutputs.ir = (m_args.count(g_strIR) > 0);
	m_options.selectedOutputs.irOptimized = (m_args.count(g_strIROptimized) > 0);
	m_options.selectedOutputs.ewasm = (m_args.count(g_strEwasm) > 0);
	m_options.selectedOutputs.signatureHashes = (m_args.count(g_strSignatureHashes) > 0);
	m_options.selectedOutputs.natspecUser = (m_args.count(g_strNatspecUser) > 0);
	m_options.selectedOutputs.natspecDev = (m_args.count(g_strNatspecDev) > 0);
	m_options.selectedOutputs.metadata = (m_args.count(g_strMetadata) > 0);
	m_options.selectedOutputs.storageLayout = (m_args.count(g_strStorageLayout) > 0);

	m_options.estimateGas = (m_args.count(g_strGas) > 0);

	po::notify(m_args);

	if (m_args.count(g_strBasePath))
		m_options.basePath = m_args[g_strBasePath].as<string>();
	...
	if (m_args.count(g_strAssemble) || m_args.count(g_strStrictAssembly) || m_args.count(g_strYul))
	{
		...
		m_options.inputAssemblyLanguage = m_args.count(g_strYul) ? Input::Yul : (m_args.count(g_strStrictAssembly) ? Input::StrictAssembly : Input::Assembly);
		m_options.optimize = (m_args.count(g_strOptimize) > 0);
	...
	if (m_args.count(g_strModelCheckerTimeout))
		m_options.modelCheckerSettings.timeout = m_args[g_strModelCheckerTimeout].as<unsigned>();

	m_options.metadataLiteral = (m_args.count(g_strMetadataLiteral) > 0);
	...
	m_options.experimentalViaIR = (m_args.count(g_strExperimentalViaIR) > 0);
	m_options.expectedExecutionsPerDeployment = m_args[g_strOptimizeRuns].as<unsigned>();
	...
	if (m_args.count(g_strImportAst) > 0)
		...
		m_options.errorRecovery = (m_args.count(g_strErrorRecovery) > 0);
